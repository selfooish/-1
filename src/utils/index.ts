...
export function getLevelFromPoints(points: number): { level: number; title: string; progress: number } {
  ...
  for (let i = levels.length - 1; i >= 0; i--) {
    if (points >= levels[i].min) {
      const nextMin = levels[i + 1]?.min || levels[i].min + 1000
      const progress = Math.round(((points - levels[i].min) / (nextMin - levels[i].min)) * 100)
      return { level: levels[i].level, title: levels[i].title, progress }
    }
  }
